...
export default class FetchDiaryView extends Vue {
    ...
    private async create() {
        axios.defaults.headers.common.Authorization = `Bearer ${localStorage.getItem('token')}`;
        const page: PageRequest = new PageRequest(this.$data.lastNumber + 1, this.$data.name);
        await axios
    ...
    private async edit() {
        axios.defaults.headers.common.Authorization = `Bearer ${localStorage.getItem('token')}`;
        const page: Page = new Page(this.$data.pages[this.$data.currentPageNum].id, this.$data.currentPageNum + 1, this.$data.currentPage);
        await axios
    ...
    private async deletePage() {
        ...
        const id = this.$data.pages[this.$data.currentPageNum].id;
    ...
    private async mark() {
        ...
        const text = this.$data.selected;
        const start = this.$data.currentPage.indexOf(text);
        const end = start + text.length;
        axios.defaults.headers.common.Authorization = `Bearer ${localStorage.getItem('token')}`;
        const marking: Marking = new Marking(this.$data.currentPageId, start, end, '');
        await axios
            .post('api/Markings/',
                marking,
                {headers: {'Content-Type': 'application/json'}})
